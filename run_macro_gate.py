...
Dark theme (#0b0e17), deployment score as huge number,
...
def load_signals():
    """Fetch all 6 signals. Returns list of result dicts."""
    results = []
    ...
    results.append(vix_level.score(vix_data))
    results.append(vix_term_structure.score(vix_data, vix3m_data))

    with st.spinner("Computing market breadth (downloads ~100 stocks)..."):
    ...
    comp = composite_mod.compute(results)
    return results, comp


@st.cache_data(ttl=3600, show_spinner=False)  # 1-hour cache for backtest
def load_backtest():
    return bt.run_backtest(lookback_days=504)  # ~2 years trading days
...
def main():
    ...
    with info_col:
        st.markdown(f"#### Signal Contributions")
    ...
    signal_order = [
        "VIX Level", "VIX Term Structure", "Market Breadth",
        "Credit Spreads", "Put/Call Sentiment", "Factor Crowding"
    ]
    ...
    for col, sig_name in zip(gauge_cols, signal_order):
        ...
        with col:
            st.plotly_chart(
                make_gauge(s, sig_name.replace(" ", "<br>")),
                use_container_width=True,
                config={"displayModeBar": False},
            )
            st.markdown(
                f'<div class="signal-detail" style="text-align:center;">{detail}</div>',
                unsafe_allow_html=True,
            )
    ...
    if bt_df is not None:
        # SPY chart color-coded by zone
        fig_spy = go.Figure()

        # Add shaded backgrounds per zone
            ...
            for date, flag in mask.items():
                if flag and not in_zone:
                    ...
                    fig_spy.add_vrect(
                        ...
                        fillcolor=z_color, opacity=0.12,
                        layer="below", line_width=0,
                    )
                    in_zone = False
            if in_zone and start_date:
                fig_spy.add_vrect(
                    x0=start_date, x1=bt_df.index[-1],
                    fillcolor=z_color, opacity=0.12,
                    layer="below", line_width=0,
                )
        ...
        # Overlay composite score on secondary axis
        fig_spy.add_trace(go.Scatter(
            x=bt_df.index,
            y=bt_df["composite_score"],
        ...
        for zone_name, z_color in ZONE_COLORS.items():
            fig_spy.add_annotation(
                ...
                y=1.04,
                showarrow=False,
                font=dict(size=10, color=z_color),
            )
        ...
        # Cumulative return by zone comparison
        zone_cum = {}
